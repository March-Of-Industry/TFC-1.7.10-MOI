    ...
    public EntityAnimal getBird() {
        List list = worldObj.getEntitiesWithinAABB(
                ...
                AxisAlignedBB.getBoundingBox(
                        xCoord + 0.1, yCoord, zCoord + 0.1, xCoord + 0.9, yCoord + 1.1, zCoord + 0.9));
        ...
        if (!list.isEmpty()) {
            for (Object e : list) {
                if (((EntityChickenTFC) e).getGender() == GenderEnum.FEMALE
                        && ((EntityChickenTFC) e).isAdult()
            ...
            for (int i = 0; i < this.getSizeInventory(); i++) {
                if (inventory[i] != null) {
                    if (inventory[i].getTagCompound() != null
                            && inventory[i].getTagCompound().hasKey("Fertilized")) {
                        ...
                        if (time <= TFC_Time.getTotalTicks()) {
                            EntityChickenTFC chick = new EntityChickenTFC(
                                    worldObj, xCoord + 0.5, yCoord + 1, zCoord + 0.5, (NBTTagCompound)
                                            inventory[i].getTagCompound().getTag("Genes"));
