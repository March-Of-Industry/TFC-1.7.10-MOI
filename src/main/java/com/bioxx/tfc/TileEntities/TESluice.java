...
public class TESluice extends TileEntity implements IInventory {
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote) {
            ...
            for (Iterator iterator = list.iterator(); iterator.hasNext(); ) {
                ...
                if (item == Item.getItemFromBlock(TFCBlocks.gravel)
                        || item == Item.getItemFromBlock(TFCBlocks.gravel2)
            ...
            if (soilAmount > 0 && waterInput && waterOutput) {
                ...
                while (processTimeRemaining > 100 && soilAmount > 0) {
                    ...
                    if (random.nextInt((int) (200 * oreMod)) == 0 && !coreSampleStacks.isEmpty())
                        addToInventory(coreSampleStacks
                                .get(random.nextInt(coreSampleStacks.size()))
                                .copy());
        ...
        boolean isOutputAir = worldObj.isAirBlock(
                xCoord + dir.getOpposite().offsetX * 2, yCoord - 1, zCoord + dir.getOpposite().offsetZ * 2);
        boolean isOutputWater = TFC_Core.isWater(worldObj.getBlock(
                xCoord + dir.getOpposite().offsetX * 2, yCoord - 1, zCoord + dir.getOpposite().offsetZ * 2));
        boolean isWaterDepth7 = worldObj.getBlockMetadata(xCoord + dir.offsetX, yCoord + 1, zCoord + dir.offsetZ) == 7;
        int meta2 = worldObj.getBlockMetadata(
                xCoord + dir.getOpposite().offsetX, yCoord, zCoord + dir.getOpposite().offsetZ);
        if (isInputWater && isWaterDepth7 && !isFlowing && (isOutputAir || isOutputWater)) {
            ...
            if ((worldObj.getBlockMetadata(
                                    xCoord + dir.getOpposite().offsetX, yCoord, zCoord + dir.getOpposite().offsetZ)
                            & 4)
                    == 0) {
                // set second block to water on
                worldObj.setBlockMetadataWithNotify(
                        xCoord + dir.getOpposite().offsetX, yCoord, zCoord + dir.getOpposite().offsetZ, meta2 + 4, 3);
            }

            // Set output water
            worldObj.setBlock(
                    xCoord + dir.getOpposite().offsetX * 2, yCoord - 1, zCoord + dir.getOpposite().offsetZ * 2, water);
        }
