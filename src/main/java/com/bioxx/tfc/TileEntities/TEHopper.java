...
public class TEHopper extends NetworkTileEntity implements IHopper {
        ...
        if (this.worldObj.isRemote) {
            ...
            if (pressCooldown > 0) {
                ...
                for (int i = 0; i < storage.length; i++) {
                    if (storage[i] == null
                            || ItemStack.areItemStacksEqual(storage[i], pressBlock)
            ...
            if (blockAbove != null && this.hasPressableItem() > 0) {
                if (pressBlock != null
                        && !(blockAbove instanceof BlockCobble
                                || blockAbove instanceof BlockGravel
    ...
    public static IInventory getInputInventory(IHopper hopper) {
        return searchForOutputInventory(
                hopper.getWorldObj(), hopper.getXPos(), hopper.getYPos() + 1.0D, hopper.getZPos());
    }
