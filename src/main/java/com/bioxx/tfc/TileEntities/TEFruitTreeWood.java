...
public class TEFruitTreeWood extends TileEntity implements IInventory {
    ...
    public void updateEntity() {
        if (!worldObj.isRemote) {
            ...
            FloraIndex fi = manager.findMatchingIndex(
                    BlockFruitWood.getType(worldObj.getBlockMetadata(xCoord, yCoord, zCoord)));

            int meta = worldObj.getBlockMetadata(xCoord, yCoord, zCoord);
            float temp = TFC_Climate.getHeightAdjustedTemp(worldObj, xCoord, yCoord, zCoord);

            int month =
                    TFC_Time.getSeasonAdjustedMonth(zCoord); // 0 is Early Spring, 3 is Early Summer, 6 is Early Autumn
            if (month < 9
                    && fi != null
                    && temp >= fi.minTemp
                    && temp < fi.maxTemp) // Will not grow during winter months or out of temp range
            {
                int t = 1;
                ...
                if (birthTimeWood + TRUNK_GROW_TIME < TFC_Time.getTotalDays()
                        && height < 3
                        && isTrunk
                    ...
                    if (worldObj.getTileEntity(xCoord, yCoord + 1, zCoord) instanceof TEFruitTreeWood) {
                        TEFruitTreeWood trunkTE =
                                ((TEFruitTreeWood) worldObj.getTileEntity(xCoord, yCoord + 1, zCoord));
                ...
                else if (birthTimeWood + BRANCH_GROW_TIME < TFC_Time.getTotalDays()
                        && height == 2
                        && isTrunk
                        && worldObj.rand.nextInt(16 / t) == 0
                        && worldObj.getBlock(xCoord, yCoord + 1, zCoord)
                                == TFCBlocks.fruitTreeWood) // Only grow branches if the tree is full height
                {
                    ...
                    if (r == 0
                        ...
                        if (worldObj.getTileEntity(xCoord + 1, yCoord, zCoord) instanceof TEFruitTreeWood) {
                            TEFruitTreeWood branchTE =
                                    ((TEFruitTreeWood) worldObj.getTileEntity(xCoord + 1, yCoord, zCoord));
                        ...
                        if (worldObj.getTileEntity(xCoord, yCoord, zCoord - 1) instanceof TEFruitTreeWood) {
                            TEFruitTreeWood branchTE =
                                    ((TEFruitTreeWood) worldObj.getTileEntity(xCoord, yCoord, zCoord - 1));
                            branchTE.setupBirth(false, height, birthTimeWood + BRANCH_GROW_TIME, birthTimeLeaves);
                        }
                    } else if (r == 2
                            && worldObj.blockExists(xCoord - 1, yCoord, zCoord)
    ...
    private boolean checkLeaves(int xCoord, int yCoord, int zCoord) {
        return worldObj.blockExists(xCoord, yCoord, zCoord)
                && worldObj.isAirBlock(xCoord, yCoord, zCoord)
                && worldObj.isAirBlock(xCoord, yCoord + 1, zCoord)
