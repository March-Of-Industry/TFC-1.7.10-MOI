    ...
    public boolean canLight() {
        if (!worldObj.isRemote) {
            ...
            if (bid == TFCBlocks.charcoal && worldObj.getBlockMetadata(x, yCoord, z) >= 7 && !bloomeryLit) {
                bloomeryLit = true;
                this.fuelTimeLeft =
                        (long) (TFC_Time.getTotalTicks() + (TFC_Time.HOUR_LENGTH * TFCOptions.bloomeryBurnTime));
    ...
    public void updateEntity() {
        if (!worldObj.isRemote) {
            ...
            if (worldObj.blockExists(x, yCoord, z)) {
                ...
                for (int i = bloomeryLit ? 0 : 1, j = bloomeryLit ? moltenHeight + 7 : moltenHeight;
                        j > 0;
                        i++, j -= 8) {
                    ...
                    for (Iterator iterator = list.iterator(); iterator.hasNext(); ) {
                        ...
                        if (entity.getEntityItem().getItem() == TFCItems.coal
                                && entity.getEntityItem().getItemDamage()
                                        == 1 /*|| entity.getEntityItem().getItem() == TFCItems.Coke*/) {
                        ...
                        /*If the item that's been tossed in is a type of Ore and it can melt down into something then add the ore to the list of items in the fire.*/
                        else if (entity.getEntityItem().getItem() instanceof ItemOre
                                && ((ItemOre) entity.getEntityItem().getItem()).isSmeltable(entity.getEntityItem())) {
                            int c = entity.getEntityItem().stackSize;
                            while (c > 0) {
                                if (charcoalCount + oreCount < (2 * maxCount)
                                        && oreCount < maxCount
                                        && outCount < 1000) {
                                    if (addOreToFire(new ItemStack(
                            ...
                            if (c == 0) entity.setDead();
                                ...
                                && ((ISmeltable) entity.getEntityItem().getItem())
                                        .isSmeltable(entity.getEntityItem())) {
                            int c = entity.getEntityItem().stackSize;
                            while (c > 0) {
                                if (((ISmeltable) entity.getEntityItem().getItem())
                                                        .getMetalReturnAmount(entity.getEntityItem())
                                                < 100
                                        && oreCount < maxCount
                                        && outCount < 1000) {
                                    if (addOreToFire(new ItemStack(
