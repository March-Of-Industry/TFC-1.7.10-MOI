...
public class TEAnvil extends NetworkTileEntity implements IInventory {
            ...
            if (workRecipe != null && getItemCraftingValue() != itemCraftingValue) {
                ...
                if (result != null) {
                    AnvilCraftEvent eventCraft = new AnvilCraftEvent(
                            lastWorker, this, anvilItemStacks[INPUT1_SLOT], anvilItemStacks[INPUT2_SLOT], result);
    ...
    public void updateRecipe() {
        ...
        if (anvilItemStacks[INPUT1_SLOT] != null
                && anvilItemStacks[INPUT1_SLOT].getItem() == TFCItems.bloom
                && workRecipe.getCraftingResult().getItem() == TFCItems.bloom) {
    ...
    private boolean canBeWorked() {
        return isTemperatureWorkable(INPUT1_SLOT)
                && anvilItemStacks[HAMMER_SLOT] != null
                && (anvilItemStacks[INPUT1_SLOT].getItemDamage() == 0
                        || anvilItemStacks[INPUT1_SLOT].getItem().getHasSubtypes())
    ...
    public void actionWeld() {
        if (!worldObj.isRemote) {
            if (isTemperatureWeldable(WELD1_SLOT)
                    && isTemperatureWeldable(WELD2_SLOT)
                    && anvilItemStacks[HAMMER_SLOT] != null
                    && (anvilItemStacks[WELD1_SLOT].getItemDamage() == 0
                            || anvilItemStacks[WELD1_SLOT].getItem().getHasSubtypes())
                    && (anvilItemStacks[WELD2_SLOT].getItemDamage() == 0
                            || anvilItemStacks[WELD2_SLOT].getItem().getHasSubtypes())
                    && workedRecently == 0
    ...
    public void handleDataPacket(NBTTagCompound nbt) {
        ...
        if (soundFlag) {
            worldObj.playSoundEffect(
                    xCoord, yCoord, zCoord, TFC_Sounds.METALIMPACT, 0.1F, 0.1F + (worldObj.rand.nextFloat() / 4));
        }
    }
