        ...
        if (!worldObj.isRemote) {
            ...
            if (crop != null && growthTimer < time && sunLevel > 0) {
                ...
                float ambientTemp = TFC_Climate.getHeightAdjustedTempSpecificDay(
                        worldObj, TFC_Time.getDayOfYearFromTick(growthTimer), xCoord, yCoord, zCoord);
                float tempAdded = 0;
                ...
                if ((TFCOptions.enableCropsDie || !TFC_Core.isFarmland(worldObj.getBlock(xCoord, yCoord - 1, zCoord)))
                                && (crop.maxLifespan == -1
                                        && growth > crop.numGrowthStages + ((float) crop.numGrowthStages / 2))
                        || growth < 0) {
                    killCrop(crop);
                }
    ...
    public void onHarvest(World world, EntityPlayer player, boolean isBreaking) {
        if (!world.isRemote) {
            ...
            if (crop != null && growth >= crop.numGrowthStages - 1) {
                ...
                int skill =
                        20 - (int) (20 * TFC_Core.getSkillStats(player).getSkillMultiplier(Global.SKILL_AGRICULTURE));
