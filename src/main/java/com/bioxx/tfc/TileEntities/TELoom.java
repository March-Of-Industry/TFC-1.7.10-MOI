...
public class TELoom extends NetworkTileEntity implements IInventory {
    ...
    public ResourceLocation getWoodResource() {
        return new ResourceLocation(
                Reference.MOD_ID, "textures/blocks/wood/WoodSheet/" + Global.WOOD_ALL[loomType] + ".png");
    }

    public ResourceLocation getStringResource() {
        LoomRecipe resource = null;

        if (storage[1] != null) resource = LoomManager.getInstance().findMatchingResult(storage[1]);
        else resource = LoomManager.getInstance().findPotentialRecipes(storage[0]);
