...
public abstract class NetworkTileEntity extends TileEntity {
    ...
    public void broadcastPacketInRange() {
        ...
        if (worldObj.isRemote) TerraFirmaCraft.PACKET_PIPELINE.sendToServer(this.createDataPacket());
            ...
            TerraFirmaCraft.PACKET_PIPELINE.sendToAllAround(
                    this.createDataPacket(), new TargetPoint(dim, xCoord, yCoord, zCoord, broadcastRange));
    }
    ...
    public void broadcastPacketInRange(AbstractPacket packet) {
        if (worldObj.isRemote) TerraFirmaCraft.PACKET_PIPELINE.sendToServer(packet);
            ...
            TerraFirmaCraft.PACKET_PIPELINE.sendToAllAround(
                    packet, new TargetPoint(worldObj.provider.dimensionId, xCoord, yCoord, zCoord, broadcastRange));
    }
}
