...
public class TFCChunkProviderGenerate extends ChunkProviderGenerate {
    ...
    public Chunk provideChunk(int chunkX, int chunkZ) {
        ...
        biomesForGeneration = this.worldObj
    ...
    public void populate(IChunkProvider chunkProvider, int chunkX, int chunkZ) {
        ...
        if (this.worldObj.getBiomeGenForCoords(xCoord + 16, zCoord + 16)
                instanceof TFCBiome) // Fixes ClassCastException
        {
    ...
    public static List<SpawnListEntry> getCreatureSpawnsByChunk(World world, TFCBiome biome, int x, int z) {
        ...
        if (temp > -20 && temp <= 0) {
            ...
            if (rain > 250) {
                ...
                spawnableCreatureList.add(new SpawnListEntry(
                        EntityWolfTFC.class, 2 + mountainousAreaModifier, 1, 2 + mountainousAreaModifier));
        ...
        if (temp >= 23 && temp < 44 && rain > 1500) {
            ...
            spawnableCreatureList.add(new SpawnListEntry(
                    EntityChickenTFC.class, 3 + mountainousAreaModifier, 1, 4 + mountainousAreaModifier));
        }
        // Swamp
    ...
    public boolean canSnowAt(World world, int x, int y, int z) {
        ...
        if (var5 >= 0F) {
            ...
            if (y >= 0
                ...
                if (var7.isAir(world, x, y, z)
                        && TFCBlocks.snow.canPlaceBlockAt(world, x, y, z)
                        && !var6.isAir(world, x, y - 1, z)
                        && var6.getMaterial().blocksMovement()) return true;
            }
            return false;
        }
    }
    ...
    public void generateTerrainHigh(int chunkX, int chunkZ, Block[] idsTop) {
        ...
        this.noiseArray = this.initializeNoiseFieldHigh(
                this.noiseArray, chunkX * subDivXZ, 0, chunkZ * subDivXZ, xSize, ySize, zSize);
     ...
     * size.
     */
    private double[] initializeNoiseFieldHigh(
            double[] outArray, int xPos, int yPos, int zPos, int xSize, int ySize, int zSize) {
            ...
            for (int counter1 = -2; counter1 <= 2; ++counter1) {
                for (int counter2 = -2; counter2 <= 2; ++counter2) {
                    float parabolaHeight =
                            10.0F / MathHelper.sqrt_float(counter1 * counter1 + counter2 * counter2 + 0.2F);
        ...
        this.noise3 = this.noiseGen3.generateNoiseOctaves(
                this.noise3, xPos, yPos, zPos, xSize, ySize, zSize, double1 / 80.0D, double2 / 160.0D, double1 / 80.0D);
        ...
        this.noise1 = this.noiseGen1.generateNoiseOctaves(
                this.noise1, xPos, yPos, zPos, xSize, ySize, zSize, double1, double2, double1);
        this.noise2 = this.noiseGen2.generateNoiseOctaves(
                this.noise2, xPos, yPos, zPos, xSize, ySize, zSize, double1, double2, double1);
        ...
        for (int x = 0; x < xSize; ++x) {
            for (int z = 0; z < zSize; ++z) {
                ...
                if (scaledNoise6Value
                        < 0.0D) // Only true when noise6[index2] is between -17,777 and 0, scaledNoise6Value will be at
                // maximum -2
                {
    ...
    private void replaceBlocksForBiomeHigh(
            int chunkX, int chunkZ, Block[] idsTop, Random rand, Block[] idsBig, byte[] metaBig) {
        int seaLevel = 16;
        int worldHeight = 256;
        int indexOffset = 128;
        double var6 = 0.03125D;
        stoneNoise = noiseGen4.generateNoiseOctaves(
                stoneNoise, chunkX * 16, chunkZ * 16, 0, 16, 16, 1, var6 * 4.0D, var6 * 1.0D, var6 * 4.0D);
        ...
        for (int xCoord = 0; xCoord < 16; ++xCoord) {
            for (int zCoord = 0; zCoord < 16; ++zCoord) {
                ...
                float rain = rainfallLayer[arrayIndexDL] == null
                        ? DataLayer.RAIN_125.floatdata1
                        : rainfallLayer[arrayIndexDL].floatdata1;
                DataLayer drainage =
                        drainageLayer[arrayIndexDL] == null ? DataLayer.DRAINAGE_NORMAL : drainageLayer[arrayIndexDL];
                ...
                for (int height = 127; height >= 0; --height) {
                    ...
                    if (idsBig[indexBig] == null) {
                        ...
                        if (indexBig + 1 < idsBig.length
                                && TFC_Core.isSoilOrGravel(idsBig[indexBig + 1])
                    ...
                    if (idsBig[indexBig] == Blocks.stone) {
                        ...
                        if (var13 == -1) {
                            ...
                            for (int counter = 1; counter < var12Temp / 3; counter++) {
                                if (arrayIndexx >= 0
                                        && seaLevelOffsetMap[arrayIndex] - (3 * counter)
                            ...
                            if (var13 > 0) {
                                if (height >= seaLevel - 1
                                    ...
                                    for (int c = 1;
                                            c < dirtH
                                                    && !TFC_Core.isMountainBiome(biome.biomeID)
                    ...
                    } else if (idsTop[index] == TFCBlocks.saltWaterStationary
