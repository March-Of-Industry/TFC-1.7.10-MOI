...
public final class SpawnerAnimalsTFC {
    ...
    public static void performWorldGenSpawning(
            World world, TFCBiome biome, int par2, int par3, int par4, int par5, Random par6Random) {
        List list = TFCChunkProviderGenerate.getCreatureSpawnsByChunk(
                world, biome, par2, par3); // par1BiomeGenBase.getSpawnableList(EnumCreatureType.creature);
        if (!list.isEmpty()) {
                ...
                for (int j2 = 0; j2 < i1; ++j2) {
                    ...
                    for (int k2 = 0; !flag && k2 < 4; ++k2) {
                        ...
                        if (canCreatureTypeSpawnAtLocation(EnumCreatureType.creature, world, j1, l2, k1)) {
                            ...
                            try {
                                entityliving = (EntityLiving) spawnlistentry
                                        .entityClass
                            ...
                            if (entityliving instanceof EntityFishTFC) {
                                if (entityliving.getRNG().nextInt((int) ChunkData.FISH_POP_MAX)
                                        > TFC_Core.getCDM(world).getFishPop(j1 >> 4, k1 >> 4)) {
                                    return;
                                }
                            }
