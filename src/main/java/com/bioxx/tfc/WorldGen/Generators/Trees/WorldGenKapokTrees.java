...
public class WorldGenKapokTrees extends WorldGenerator {
    ...
    private static final byte[] OTHER_COORD_PAIRS =
            new byte[] {(byte) 2, (byte) 0, (byte) 0, (byte) 1, (byte) 2, (byte) 1};
    ...
    private void generateBranchPoints() {
        double branchSlope =
                0; // starts out horizontal and becomes more vertical. should be 0.2-1.5, trying out constant 0.5
    ...
    private void genTreeLayer(int par1, int par2, int par3, float par4, byte par5, Block b) {
        ...
        for (var11[par5] = var10[par5]; var12 <= var7; ++var12) {
            ...
            while (var13 <= var7) {
                double var15 =
                        Math.sqrt(Math.pow(Math.abs(var12) + 0.5D, 2.0D) + Math.pow(Math.abs(var13) + 0.5D, 2.0D));
    ...
    private void placeBlockLine(int[] par1ArrayOfInteger, int[] par2ArrayOfInteger, Block par3) {
        ...
        if (var4[var6] != 0) {
            ...
            if (par1ArrayOfInteger[0] == par2ArrayOfInteger[0]
                    && par1ArrayOfInteger[2] == par2ArrayOfInteger[2]
    ...
    private void placeBlockLineCurved(int[] startingPos, int[] endingPos, Block par3) {
        int[] midPointRaised = {
            (endingPos[0] - startingPos[0]) / 2,
            (int) ((endingPos[1] - startingPos[1]) / 1.5D),
            (endingPos[1] - startingPos[1]) / 2
        };
        int[] newDest = {
            startingPos[0] + midPointRaised[0], startingPos[1] + midPointRaised[1], startingPos[2] + midPointRaised[2]
        };
