...
public class WorldGenFissure implements IWorldGenerator {
    ...
    public void generate(World world, Random rand, int x, int y, int z) {
        ...
        for (int d = 1; d <= poolDepth; d++) {
            if (!world.getBlock(x, y - d, z).isNormalCube()) return;
        }
    ...
    private void carve(World world, int x, int y, int z, Block block, int meta) {
        ...
        if (world.getBlock(x - 1, y, z).getMaterial() != Material.air
                && TFC_Core.isRawStone(world.getBlock(x - 1, y, z))) world.setBlock(x - 1, y, z, block, meta, 3);
        if (world.getBlock(x + 1, y, z).getMaterial() != Material.air
                && TFC_Core.isRawStone(world.getBlock(x + 1, y, z))) world.setBlock(x + 1, y, z, block, meta, 3);
        if (world.getBlock(x, y, z - 1).getMaterial() != Material.air
                && TFC_Core.isRawStone(world.getBlock(x, y, z - 1))) world.setBlock(x, y, z - 1, block, meta, 3);
        if (world.getBlock(x, y, z + 1).getMaterial() != Material.air
                && TFC_Core.isRawStone(world.getBlock(x, y, z + 1))) world.setBlock(x, y, z + 1, block, meta, 3);
    }
    ...
    private void fill(World world, int x, int y, int z, Block block, int meta, Block fill) {
        ...
        if (world.getBlock(x - 1, y, z).getMaterial() == Material.air) world.setBlock(x - 1, y, z, block, meta, 2);
        if (world.getBlock(x + 1, y, z).getMaterial() == Material.air) world.setBlock(x + 1, y, z, block, meta, 2);
        if (world.getBlock(x, y, z - 1).getMaterial() == Material.air) world.setBlock(x, y, z - 1, block, meta, 2);
        if (world.getBlock(x, y, z + 1).getMaterial() == Material.air) world.setBlock(x, y, z + 1, block, meta, 2);
        if (world.getBlock(x, y - 1, z).getMaterial() == Material.air) world.setBlock(x, y - 1, z, block, meta, 2);
    }
    ...
    private void makeTunnel(Random random, World world, int x, int z, int y, BlockMeta rockLayer) {
        ...
        while (yCoord > minTunnel) {
            ...
            if (fillBlock != null
                    && world.getBlock(xCoord + 1, yCoord, zCoord).getMaterial() != fillBlock.getMaterial())
                world.setBlock(xCoord + 1, yCoord, zCoord, rockLayer.block, rockLayer.meta, 2);
            if (fillBlock != null
                    && world.getBlock(xCoord - 1, yCoord, zCoord).getMaterial() != fillBlock.getMaterial())
                world.setBlock(xCoord - 1, yCoord, zCoord, rockLayer.block, rockLayer.meta, 2);
            if (fillBlock != null
                    && world.getBlock(xCoord, yCoord, zCoord + 1).getMaterial() != fillBlock.getMaterial())
