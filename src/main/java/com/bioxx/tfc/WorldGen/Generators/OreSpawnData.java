    ...
    public OreSpawnData(String t, String s, String blockName, int m, int r, String[] baseRocks) {
        ...
        if (block == null) {
            TerraFirmaCraft.LOG.error(
                    TFC_Core.translate("error.error") + " " + TFC_Core.translate("error.OreCFG") + " " + blockName);
    ...
    private void getOre(String name) {
        for (int i = 0; i < Global.STONE_IGIN.length; i++) {
            if (name.equalsIgnoreCase(Global.STONE_IGIN[i])) {
                List<Integer> metadata = base.containsKey(TFCBlocks.stoneIgIn)
                        ? base.get(TFCBlocks.stoneIgIn)
        ...
        for (int i = 0; i < Global.STONE_IGEX.length; i++) {
            if (name.equalsIgnoreCase(Global.STONE_IGEX[i])) {
                List<Integer> metadata = base.containsKey(TFCBlocks.stoneIgEx)
                        ? base.get(TFCBlocks.stoneIgEx)
                        : new ArrayList<Integer>();
                metadata.add(i);
        ...
        for (int i = 0; i < Global.STONE_SED.length; i++) {
            if (name.equalsIgnoreCase(Global.STONE_SED[i])) {
                List<Integer> metadata =
                        base.containsKey(TFCBlocks.stoneSed) ? base.get(TFCBlocks.stoneSed) : new ArrayList<Integer>();
                metadata.add(i);
        ...
        for (int i = 0; i < Global.STONE_MM.length; i++) {
            if (name.equalsIgnoreCase(Global.STONE_MM[i])) {
                List<Integer> metadata =
                        base.containsKey(TFCBlocks.stoneMM) ? base.get(TFCBlocks.stoneMM) : new ArrayList<Integer>();
                metadata.add(i);
        ...
        if ("igneous intrusive".equalsIgnoreCase(name)) {
            List<Integer> metadata =
                    base.containsKey(TFCBlocks.stoneIgIn) ? base.get(TFCBlocks.stoneIgIn) : new ArrayList<Integer>();
            metadata.add(-1);
            base.put(TFCBlocks.stoneIgIn, metadata);
            return;
        } else if ("igneous extrusive".equalsIgnoreCase(name)) {
            List<Integer> metadata =
                    base.containsKey(TFCBlocks.stoneIgEx) ? base.get(TFCBlocks.stoneIgEx) : new ArrayList<Integer>();
            metadata.add(-1);
            base.put(TFCBlocks.stoneIgEx, metadata);
            return;
