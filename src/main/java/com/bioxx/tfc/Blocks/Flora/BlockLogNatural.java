    ...
    public void updateTick(World world, int x, int y, int z, Random rand) {
        if (!world.isRemote) {
            if (!world.getBlock(x, y - 1, z).isOpaqueCube()) {
                if (noLogsNearby(world, x + 1, y, z)
                        && noLogsNearby(world, x - 1, y, z)
            ...
            if (equip != null) {
                ...
                if (isAxe) {
                    ...
                    EntityItem item = new EntityItem(
                            world, x + 0.5, y + 0.5, z + 0.5, new ItemStack(TFCItems.stick, 1 + world.rand.nextInt(3)));
        ...
        if (y >= 0 && j + y < 256) {
            ...
            if (stack != null) {
                if (damage + stack.getItemDamage() <= stack.getMaxDamage()) {
                    ...
                    if (logs >= 16) {
                        dropBlockAsItem(
                                world, i + x, j + y, k + z, new ItemStack(TFCItems.logs, 16, damageDropped(meta)));
                        logs -= 16;
                    }
