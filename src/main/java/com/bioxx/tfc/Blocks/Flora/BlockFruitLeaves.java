...
public class BlockFruitLeaves extends BlockTerraContainer {
    ...
    public void registerBlockIcons(IIconRegister iconRegisterer) {
        for (int i = 0; i < 9; i++) {
            icons[i] = iconRegisterer.registerIcon(
                    Reference.MOD_ID + ":" + "wood/fruit trees/" + woodNames[i] + " Leaves");
            iconsOpaque[i] = iconRegisterer.registerIcon(
                    Reference.MOD_ID + ":" + "wood/fruit trees/" + woodNames[i] + " Leaves Opaque");
            ...
            iconsFruit[i] =
                    iconRegisterer.registerIcon(Reference.MOD_ID + ":" + "wood/fruit trees/" + woodNames[i] + " Fruit");
            iconsFlowers[i] = iconRegisterer.registerIcon(
                    Reference.MOD_ID + ":" + "wood/fruit trees/" + woodNames[i] + " Flowers");
        }
    }
            ...
            if (te != null) {
                if (fi2 != null) {
                    if (temp >= fi2.minTemp && temp < fi2.maxTemp) {
                        if (fi2.inHarvest(TFC_Time.getSeasonAdjustedMonth(z))
                                && !te.hasFruit
            ...
            for (int j = 0; j >= -1; j--) {
                for (int k = 1; k >= -1; k--) {
                    if (world.getBlock(i + x, j + y, k + z) == TFCBlocks.fruitTreeWood
                            && world.getBlock(i + x, j + y + 1, k + z)
                                    != TFCBlocks.fruitTreeWood) // Only branches or the top of the trunk
                    return true;
                }
            }
        }
    ...
    public void onBlockClicked(World world, int x, int y, int z, EntityPlayer entityplayer) {
        if (!world.isRemote) {
            ...
            if (fi != null
                    && (fi.inHarvest(TFC_Time.getSeasonAdjustedMonth(z))
                ...
                if (te != null && te.hasFruit) {
                    ...
                    dropBlockAsItem(
                            ...
                            ItemFoodTFC.createTag(
                                    fi.getOutput(), Helper.roundNumber(4 + (world.rand.nextFloat() * 12), 10)));
                }
            }
        }
    }
