...
public class BlockSulfur extends BlockTerra implements ICustomCollision {
    ...
    public void onNeighborBlockChange(World world, int i, int j, int k, Block l) {
        int num = 0;
        if (world.getBlock(i, j, k + 1).isSideSolid(world, i, j, k + 1, ForgeDirection.NORTH)) num++;
        if (world.getBlock(i, j, k - 1).isSideSolid(world, i, j, k - 1, ForgeDirection.SOUTH)) num++;
        if (world.getBlock(i + 1, j, k).isSideSolid(world, i + 1, j, k, ForgeDirection.WEST)) num++;
        if (world.getBlock(i - 1, j, k).isSideSolid(world, i - 1, j, k, ForgeDirection.EAST)) num++;
        if (world.getBlock(i, j + 1, k).isSideSolid(world, i, j + 1, k, ForgeDirection.DOWN)) num++;
        if (world.getBlock(i, j - 1, k).isSideSolid(world, i, j - 1, k, ForgeDirection.UP)) num++;
        if (num == 0) {
    ...
    public void addCollisionBoxesToList(
            World world, int i, int j, int k, AxisAlignedBB aabb, List list, Entity entity) {
