...
public class BlockStalactite extends BlockTerra {
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess access, int i, int j, int k) {
        ...
        r = new Random(i + (i * k));
        if (isStalac) {
            float height = TFC_Core.isRawStone(access.getBlock(i, j + 1, k))
                    ? 1
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool(World world, int i, int j, int k) {
        ...
        r = new Random(i + (i * k));
        if (isStalac) {
            float height = TFC_Core.isRawStone(world.getBlock(i, j + 1, k))
                    ? 1
        ...
        return AxisAlignedBB.getBoundingBox(
                i + this.minX, j + this.minY, k + this.minZ, i + this.maxX, j + this.maxY, k + this.maxZ);
    }
