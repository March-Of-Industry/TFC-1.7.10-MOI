...
public class BlockGrill extends BlockTerraContainer {
    ...
    public boolean onBlockActivated(
        ...
        if (entityplayer.inventory.getCurrentItem() != null
                && entityplayer.inventory.getCurrentItem().getItem() instanceof ItemMetalTrapDoor) return false;
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess access, int x, int y, int z) {
        if (access.getTileEntity(x, y, z) != null && access.getTileEntity(x, y, z) instanceof TEGrill)
            this.setBlockBoundsForBlockRender(
                    access.getBlockMetadata(x, y, z), ((TEGrill) access.getTileEntity(x, y, z)).data);
    }
    ...
    public IIcon getIcon(int side, int meta) {
        if (meta == 0) {
            if (
            /*TFCOptions.use2DGrill && */ side == 0 || side == 1) {
                return blockIcon;
            }
