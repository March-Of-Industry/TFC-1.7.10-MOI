...
public class BlockLeatherRack extends BlockTerraContainer {
    ...
    public BlockLeatherRack() {
        ...
        this.setBlockBounds(
                0, 0, 0, 1, 0.001f, 1); // Very flat, but still has a hitbox that can be highlighted with the cursor.
    }
    ...
    public boolean onBlockActivated(
        ...
        if (!world.isRemote
                && world.getTileEntity(x, y, z)
                        instanceof TELeatherRack) // Instanceof check to avoid TE casting crashes.
        {
            ...
            if (te.workedArea != -1
                    && equipped != null
                    && equipped.getItem() instanceof IKnife) // The player is trying to scrape a piece off the hide.
            {
                int coord = (int) Math.floor(hitX / 0.25f)
                        + (int) Math.floor(hitZ / 0.25f) * 4; // Get location of spot being scraped.
            ...
            // hide.
            {
                world.setBlockToAir(
                        x, y,
                        z); // Spawns any existing hides from the block as entities, and destroys the block and TE.
            }
        }

        if (!canBlockStay(
                world, x, y,
                z)) // For when players attempt to scrape a leather rack that should not exist (cheated in, no log
        // underneath, etc).
        {
    ...
    public boolean canBlockStay(World world, int x, int y, int z) {
        ...
        return world.getBlock(x, y - 1, z).getMaterial()
                == Material.wood; // Leather racks without a log underneath should not exist.
    }
