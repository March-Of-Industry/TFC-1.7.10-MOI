    ...
    public boolean onBlockActivated(
        ...
        if (world.isRemote) {
            ...
            if (world.getTileEntity(x, y, z) instanceof TEBarrel) {
                ...
                if (this == TFCBlocks.barrel
                        && te.getSealed()
                        && te.getGunPowderCount() >= 12
                        && player.getCurrentEquippedItem() != null
                        && player.getCurrentEquippedItem().getItem() instanceof ItemFlintAndSteel) {
                    spawnPowderKeg(world, x, y, z, te, false);
                    world.setBlockToAir(x, y, z);
                    return true;
                }
    ...
    protected boolean handleInteraction(EntityPlayer player, TEBarrel te) {
        if (!te.getSealed() && te.getInvCount() <= 1 && !te.getWorldObj().isRemote) {
            ...
            if ((FluidContainerRegistry.isFilledContainer(equippedItem)
                ...
                if (equippedItem.stackSize == 0
                        && (is.getMaxStackSize() == 1
                                || !player.inventory.hasItemStack(is))) // put buckets in the slot you used them from.
                ...
                if (equippedItem.hasTagCompound()) {
                    if (equippedItem.getTagCompound().hasKey("fluidNBT")
                            && !equippedItem.getTagCompound().hasKey("Items")
                            && te.getFluidLevel() < te.getMaxLiquid()) {
                        FluidStack fs = FluidStack.loadFluidStackFromNBT(
                                equippedItem.getTagCompound().getCompoundTag("fluidNBT"));
                        if (te.addLiquid(fs)) {
                            if (fs.amount == 0) {
                                equippedItem.getTagCompound().removeTag("Sealed");
                                equippedItem.getTagCompound().removeTag("fluidNBT");
                                if (equippedItem.getTagCompound().hasNoTags()) equippedItem.setTagCompound(null);
                            } else {
