...
public abstract class BlockCustomLiquid extends BlockDynamicLiquid implements IFluidBlock {
    ...
    private void checkForHarden(World world, int x, int y, int z) {
        if (world.getBlock(x, y, z) == this)
            if (this.blockMaterial == Material.lava) {
                ...
                if (flag || world.getBlock(x, y, z - 1).getMaterial() == Material.water) flag = true;

                if (flag || world.getBlock(x, y, z + 1).getMaterial() == Material.water) flag = true;

                if (flag || world.getBlock(x - 1, y, z).getMaterial() == Material.water) flag = true;

                if (flag || world.getBlock(x + 1, y, z).getMaterial() == Material.water) flag = true;

                if (flag || world.getBlock(x, y + 1, z).getMaterial() == Material.water) flag = true;
    ...
    public boolean canSustainPlant(
        ...
        switch (plantType) {
            ...
            case Plains:
                return this == TFCBlocks.grass
                        || this == TFCBlocks.grass2
                        || this == TFCBlocks.dirt
        ...
        if (meta > 0) {
            ...
            if (this.sourceBlockCount >= 2 && this.blockMaterial == Material.water && !TFCOptions.enableFiniteWater) {
                ...
                if (world.getBlock(x, y - 1, z).getMaterial().isSolid()
                        || world.getBlock(x, y - 1, z).getMaterial() == this.blockMaterial
    ...
    private boolean[] getFlowDirections(World world, int x, int y, int z) {
        ...
        for (side = 0; side < 4; ++side) {
            ...
            if (!this.canStay(world, newX, y, newZ)
                    && (world.getBlock(newX, y, newZ).getMaterial() != this.blockMaterial
    ...
    private int getFlowPriorities(World world, int x, int y, int z, int distance, int side) {
        ...
        for (int side2 = 0; side2 < 4; ++side2) {
            if ((side2 != 0 || side != 1)
                ...
                if (!this.canStay(world, xCoord, y, zCoord)
                        && (world.getBlock(xCoord, y, zCoord).getMaterial() != this.blockMaterial
                                || world.getBlockMetadata(xCoord, y, zCoord) != 0)) {
