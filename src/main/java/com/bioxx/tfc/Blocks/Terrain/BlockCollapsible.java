...
public class BlockCollapsible extends BlockTerraContainer {
    ...
    public void dropCarvedStone(World world, int x, int y, int z) {
        if (world.getBlock(x + 1, y, z).isOpaqueCube()) return;
        else if (world.getBlock(x - 1, y, z).isOpaqueCube()) return;
        else if (world.getBlock(x, y, z + 1).isOpaqueCube()) return;
        else if (world.getBlock(x, y, z - 1).isOpaqueCube()) return;
        else if (world.getBlock(x, y + 1, z).isOpaqueCube()) return;
        else if (world.getBlock(x, y - 1, z).isOpaqueCube()) return;
    ...
    public static void updateTickCollapsible(
            World world, int i, int j, int k, Random random, Block us, boolean checkForSupport) {
        if (world.isRemote) return;

        if (canFallBelow(world, i, j - 1, k)) {
            if (checkForSupport && isNearSupport(world, i, j, k, 4, 0))
                return; // Inside the if so it's only called when needed.
    ...
    public static Boolean isNearSupport(World world, int i, int j, int k, int range, float collapseChance) {
        ...
        if (!world.doChunksNearChunkExist(i, j, k, range + 1))
            return true; // If chunks are not loaded, assume it's supported.
    ...
    public Boolean isUnderLoad(World world, int i, int j, int k) {
        for (int x = 1; x <= TFCOptions.minimumRockLoad; x++) {
            if (!world.getBlock(i, j + x, k).isOpaqueCube()) return false;
        }
        return true;
    }
    ...
    public Boolean tryToCollapse(World world, int x, int y, int z, float collapseChance) {
        ...
        if (canFallBelow(world, x, y - 1, z)
                && isUnderLoad(world, x, y, z)
                && !isNearSupport(world, x, y, z, 4, collapseChance)) {
            if (fallingBlock != null) {
                EntityFallingBlockTFC ent =
                        new EntityFallingBlockTFC(world, x + 0.5F, y + 0.5F, z + 0.5F, fallingBlock, fallingBlockMeta);
                ...
                if (this instanceof BlockStone)
                    ent = new EntityFallingBlockTFC(
                            world, x + 0.5F, y + 0.5F, z + 0.5F, fallingBlock, fallingBlockMeta + 8);
    ...
    public static void tryToFall(World world, int x, int y, int z, Block block) {
        ...
        if (canFallBelow(world, x, y - 1, z)
            ...
            if (!fallInstantly && world.checkChunksExist(x - 32, y - 32, z - 32, x + 32, y + 32, z + 32)) {
                if (!world.isRemote) {
                    EntityFallingBlockTFC entityfallingblock =
                            new EntityFallingBlockTFC(world, x + 0.5F, y + 0.5F, z + 0.5F, block, meta);
    ...
    public void harvestBlock(World world, EntityPlayer entityplayer, int x, int y, int z, int meta) {
        ...
        int finalCollapseRatio = TFCOptions.initialCollapseRatio > 0
                ? TFCOptions.initialCollapseRatio
                : 10; // Set to default if invalid value is entered in config.
    ...
    public List<ByteCoord> getCollapseMap(World world, int i, int j, int k) {
        ...
        checkQueue.add(
                new CollapseData(new ByteCoord(-1, 0, 0), TFCOptions.propogateCollapseChance, TFCDirection.WEST));
        checkQueue.add(
                new CollapseData(new ByteCoord(1, 0, 1), TFCOptions.propogateCollapseChance, TFCDirection.NORTHEAST));
        checkQueue.add(
                new CollapseData(new ByteCoord(1, 0, -1), TFCOptions.propogateCollapseChance, TFCDirection.SOUTHEAST));
        checkQueue.add(
                new CollapseData(new ByteCoord(-1, 0, 1), TFCOptions.propogateCollapseChance, TFCDirection.NORTHWEST));
        checkQueue.add(
                new CollapseData(new ByteCoord(-1, 0, -1), TFCOptions.propogateCollapseChance, TFCDirection.SOUTHWEST));
        checkQueue.add(
                new CollapseData(new ByteCoord(0, 0, 1), TFCOptions.propogateCollapseChance, TFCDirection.SOUTH));
        checkQueue.add(
                new CollapseData(new ByteCoord(0, 0, -1), TFCOptions.propogateCollapseChance, TFCDirection.NORTH));
