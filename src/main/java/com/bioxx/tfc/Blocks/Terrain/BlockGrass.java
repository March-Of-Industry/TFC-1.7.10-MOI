...
public class BlockGrass extends BlockTerra {
    ...
    public void updateTick(World world, int i, int j, int k, Random rand) {
        if (!world.isRemote) {
            ...
            if (world.getBlock(i, j + 1, k) == Blocks.snow && !TFC_Core.isDryGrass(this)) {
                ...
                if (TFC_Core.isGrass(this)
                        && !TFC_Core.isDryGrass(this)
                    ...
                    if (rand.nextInt((int) ((16800 - rain) / 4)) == 0 && temp > 20)
                        world.setBlock(
                                ...
                                k,
                                TFCBlocks.tallGrass,
                                (world.rand.nextInt(30) == 0 ? 1 : 0),
                                0x2); // 1/30 chance to spawn fern
                    ...
                    for (int x = -4; x < 5 && !nearWater; x++) {
                        for (int z = -4; z < 5 && !nearWater; z++) {
                            if (j < 250
                                    ...
                                    && world.getBlock(i + x, j - y, k + z).getMaterial() == Material.water)
                                nearWater = true;
                        }
                    }
                }
    ...
    public void onEntityWalking(World world, int x, int y, int z, Entity entity) {
        if (!world.isRemote
                && this != TFCBlocks.clayGrass2
                && this != TFCBlocks.clayGrass
                && this != TFCBlocks.peatGrass) {
            Random r = new Random();
            if (BlockCollapsible.canFallBelow(world, x, y - 1, z)
                    && r.nextInt(10) == 0
