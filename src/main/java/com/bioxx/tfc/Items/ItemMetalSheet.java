    ...
    public boolean onItemUse(
        ...
        if (!world.isRemote) {
            ...
            // Creating a new sheet block. Cannot click on a sheet block to make a new adjacent one.
                ...
                te = (TEMetalSheet) world.getTileEntity(
                        sides[0] + x, sides[1] + y, sides[2] + z); // isValid prevents ClassCastException
                te.metalID = this.metalID;
                te.sheetStack = itemstack.copy();
                te.sheetStack.stackSize =
                        1; // stackSize is always 1 until the block is broken, and then updated based on the sides.
