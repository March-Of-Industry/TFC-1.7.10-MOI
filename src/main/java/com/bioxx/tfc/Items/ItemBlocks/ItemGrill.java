...
public class ItemGrill extends ItemTerraBlock {
    ...
    public boolean checkSides(World world, int x, int y, int z) {
        ...
        if (world.getBlock(x, y, z - 1)
                .isSideSolid(
                        world, x, y, z - 1, ForgeDirection.SOUTH)) // Check the North Block if the South Side is solid
        {
            count++;
        }
        if (world.getBlock(x, y, z + 1)
                .isSideSolid(
                        world, x, y, z + 1, ForgeDirection.NORTH)) // Check the South Block if the North Side is solid
        {
            count++;
        }
        return count >= 2;
