...
public class ItemTerraTool extends ItemTool implements ISize {
    ...
    public void addInformation(ItemStack is, EntityPlayer player, List arraylist, boolean flag) {
        ...
        if (is.getItem() instanceof ICausesDamage)
            arraylist.add(EnumChatFormatting.AQUA
                    + TFC_Core.translate(((ICausesDamage) this).getDamageType().toString()));
    ...
    public static void addSmithingBonusInformation(ItemStack is, List<String> arraylist) {
        if (AnvilManager.getDurabilityBuff(is) > 0)
            arraylist.add(TFC_Core.translate("gui.SmithingBonus") + " : +"
    ...
    public void registerIcons(IIconRegister registerer) {
        this.itemIcon = registerer.registerIcon(
                Reference.MOD_ID + ":" + "tools/" + this.getUnlocalizedName().replace("item.", ""));
