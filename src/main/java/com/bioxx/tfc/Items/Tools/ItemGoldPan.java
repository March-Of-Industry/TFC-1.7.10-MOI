...
public class ItemGoldPan extends ItemTerra {
                    ...
                    if (cd == null) {
                        TFC_Core.sendInfoMessage(
                                ...
                                new ChatComponentText(
                                        "The ChunkData returned null, please report this to the developer."));
                        return is;
                    }
    ...
    private int getMetalToDrop(World world, EntityPlayer player, int x, int y, int z) {
        ...
        if (rank.ordinal() > 0 && randType > 50 && world.rand.nextInt((int) Math.floor(120 * skillMod)) == 0)
            type = 4; // Silver
        if (rank.ordinal() > 1 && randType > 75 && world.rand.nextInt((int) Math.floor(150 * skillMod)) == 0)
            type = 1; // Gold
