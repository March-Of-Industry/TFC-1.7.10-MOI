...
public class BarrelRecipe {
    ...
    public Stack<ItemStack> getResult(ItemStack inIS, FluidStack inFS, int sealedTime) {
        ...
        if (recipeOutIS != null) {
            ...
            if (remainder > 0) {
                stackList.push(new ItemStack(
                        item, remainder, damage)); // Push this on first, so it doesn't end up in the input slot.
    ...
    public FluidStack getResultFluid(ItemStack inIS, FluidStack inFS, int sealedTime) {
        if (recipeOutFluid != null) {
            ...
            if (recipeOutFluid.tag != null) {
                fs = new FluidStack(
                        recipeOutFluid.getFluid(), recipeOutFluid.amount, (NBTTagCompound) recipeOutFluid.tag.copy());
            } else {
