...
public class Helper {
    ...
    public static MovingObjectPosition getMovingObjectPositionFromPlayer(
            World world, EntityLivingBase entity, boolean scanFluids) {
        return getMovingObjectPositionFromPlayer(world, entity, scanFluids, 4);
    }

    public static MovingObjectPosition getMovingObjectPositionFromPlayer(
            World world, EntityLivingBase entity, boolean scanFluids, int reach) {
        float var4 = 1.0F;
    ...
    private static boolean usesSRG(Object obj, String srgName) {
        ...
        for (Field f : fields) {
            if (f.getName().equals(srgName)) return true;
        }
        return false;
    }
    ...
    public static int getInteger(Object obj, String srgName, String obfName, String deobfName, boolean useDeobf) {
        Field f = null;
        try {
            if (!useDeobf) f = obj.getClass().getDeclaredField(deobfName);
            else if (usesSRG(obj, srgName)) f = obj.getClass().getDeclaredField(srgName);
            else f = obj.getClass().getDeclaredField(obfName);
