...
public class WAILAData implements IWailaDataProvider {
    ...
    public ItemStack berryBushStack(IWailaDataAccessor accessor, IWailaConfigHandler config) {
        ...
        FloraIndex index =
                FloraManager.getInstance().findMatchingIndex(BlockBerryBush.metaNames[accessor.getMetadata()]);
    ...
    public List<String> fruitLeavesHead(
            ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor, IWailaConfigHandler config) {
        NBTTagCompound tag = accessor.getNBTData();
        boolean hasFruit = tag.getBoolean("hasFruit");
    ...
    public List<String> fruitTreeWoodHead(
            ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor, IWailaConfigHandler config) {
    ...
    public List<String> ingotPileHead(
            ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor, IWailaConfigHandler config) {
        String head = currenttip.get(0);
    ...
    public List<String> oreHead(
        ...
        if (accessor.getBlock() == TFCBlocks.ore) {
            if (meta == 14) {
                currenttip.set(
                        0, EnumChatFormatting.WHITE.toString() + TFC_Core.translate("item.Ore.Bituminous Coal.name"));
            } else if (meta == 15) {
    ...
    public List<String> waterPlantHead(
            ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor, IWailaConfigHandler config) {
        MovingObjectPosition pos = accessor.getPosition();
        World world = accessor.getWorld();
        ...
        if (TFC_Core.isFreshWater(blockDirectlyAbove)) {
            if (airTwoAbove) {
                currenttip.set(
                        0, EnumChatFormatting.WHITE.toString() + TFC_Core.translate("tile.Flora.Cat Tails.name"));
            } else {
                currenttip.set(
                        0, EnumChatFormatting.WHITE.toString() + TFC_Core.translate("tile.Flora.Pond Weed.name"));
            }
        }
    ...
    public List<String> barrelBody(
        ...
        if (sealed && sealTime != 0) {
            currenttip.add(
                    TFC_Core.translate("gui.Barrel.SealedOn") + " : " + TFC_Time.getDateStringFromHours(sealTime));
        }

        // Output
        BarrelPreservativeRecipe preservative =
                BarrelManager.getInstance().findMatchingPreservativeRepice(te, inStack, fluid, sealed);
    ...
    public List<String> berryBushBody(
            ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor, IWailaConfigHandler config) {
        FloraIndex index =
                FloraManager.getInstance().findMatchingIndex(BlockBerryBush.metaNames[accessor.getMetadata()]);
    ...
    public List<String> blastFurnaceBody(
            ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor, IWailaConfigHandler config) {
    ...
    public List<String> bloomeryBody(
            ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor, IWailaConfigHandler config) {
        NBTTagCompound tag = accessor.getNBTData();
        ...
        if (isLit) {
            ...
            if (hours > 0) {
                float percent =
                        Helper.roundNumber(Math.min(100 - ((hours / TFCOptions.bloomeryBurnTime) * 100), 100), 10);
    ...
    public List<String> farmlandBody(
            ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor, IWailaConfigHandler config) {
        ...
        if (rank == SkillRank.Expert || rank == SkillRank.Master) {
            ...
            for (int i = 0; i < nutrients.length; i++) {
                ...
                if (i == 0)
                    currenttip.add(
                            EnumChatFormatting.RED + TFC_Core.translate("gui.Nutrient.A") + " : " + percent + "%");
                else if (i == 1)
                    currenttip.add(
                    ...
                    currenttip.add(EnumChatFormatting.WHITE + TFC_Core.translate("item.Fertilizer.name") + " : "
                            + percent + "%");
            }
        }
    ...
    public List<String> firepitBody(
            ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor, IWailaConfigHandler config) {
        NBTTagCompound tag = accessor.getNBTData();
        ItemStack storage[] = getStorage(tag, accessor.getTileEntity());
    ...
    public List<String> fruitLeavesBody(
            ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor, IWailaConfigHandler config) {
        FloraIndex index = FloraManager.getInstance()
    ...
    public List<String> logPileBody(
            ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor, IWailaConfigHandler config) {
        NBTTagCompound tag = accessor.getNBTData();
        ...
        if (isOnFire) {
            ...
            }; // Used to keep track of which slots have already been combined into others.
            ...
            {
                if (storage[j] != null
                        && !counted[j]) // Make sure the slot is not empty, and has not already been accounted for
                {
                    ...
                    {
                        if (storage[k] != null
                                && storage[j].isItemEqual(storage[
                                        k])) // Make sure the comparison slot isn't empty, and see if it's the same type
                        // of log
                        {
    ...
    public List<String> metalTrapDoorBody(
            ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor, IWailaConfigHandler config) {
        NBTTagCompound tag = accessor.getNBTData();
    ...
    public List<String> nestBoxBody(
            ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor, IWailaConfigHandler config) {
        NBTTagCompound tag = accessor.getNBTData();
        ItemStack storage[] = getStorage(tag, accessor.getTileEntity());
        ...
        for (ItemStack is : storage) {
            if (is != null && is.getItem() == TFCItems.egg) {
                if (is.hasTagCompound() && is.getTagCompound().hasKey("Fertilized")) fertEggCount++;
                else eggCount++;
            }
        }
    ...
    public List<String> oilLampBody(
            ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor, IWailaConfigHandler config) {
        NBTTagCompound tag = accessor.getNBTData();
        if (tag.hasKey("Fuel")) {
    ...
    public List<String> oreBody(
        ...
        if (accessor.getBlock() == TFCBlocks.ore) {
            switch (meta) {
                ...
                case 2:
                    currenttip.add(
                            TFC_Core.translate("gui.metal.Platinum") + " - " + TFC_Core.translate("gui.useless"));
                    break;
                case 3:
                case 10:
                case 11:
    ...
    public List<String> potteryBody(
            ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor, IWailaConfigHandler config) {
        NBTTagCompound tag = accessor.getNBTData();
        ...
        if (straw > 0 && straw < 8) currenttip.add(TFC_Core.translate("item.Straw.name") + " : " + straw + "/8");
            ...
            int hours = (int)
                    (TFCOptions.pitKilnBurnTime - (TFC_Time.getTotalHours() - (burnStart / TFC_Time.HOUR_LENGTH)));
            currenttip.add(hours + " " + TFC_Core.translate("gui.hoursRemaining") + " ("
    ...
    public List<String> saplingBody(
            ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor, IWailaConfigHandler config) {
        NBTTagCompound tag = accessor.getNBTData();
    ...
    public List<String> smokeRackBody(
            ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor, IWailaConfigHandler config) {
        NBTTagCompound tag = accessor.getNBTData();
        ItemStack storage[] = getStorage(tag, accessor.getTileEntity());
        ...
        for (int i = 0; i < storage.length; i++) {
            ...
            if (is != null) {
                ...
                if (smokeHours < Food.SMOKEHOURS && !Food.isSmoked(is)) {
                    ...
                    currenttip.add("\u00B7 " + smokeHours + " " + TFC_Core.translate("gui.hoursRemaining") + " ("
                            + percent + "%)");
                    ...
                    currenttip.add("\u00B7 " + dryHours + " " + TFC_Core.translate("gui.hoursRemaining") + " ("
                            + percent + "%)");
    ...
    public List<String> spawnMeterBody(
            ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor, IWailaConfigHandler config) {
        NBTTagCompound tag = accessor.getNBTData();
