...
public class EntityFallingBlockTFC extends Entity implements IEntityAdditionalSpawnData {
                ...
                if (this.onGround) {
                    ...
                    if (this.worldObj.getBlock(i, j, k) != Blocks.piston_extension) {
                        ...
                        if (canPlaceEntityOnSide(
                                        worldObj, this.block, i, j, k, true, 1, (Entity) null, (ItemStack) null)
                            ...
                            if (this.tileEntityData != null && this.block instanceof ITileEntityProvider) {
                                ...
                                if (tileentity != null) {
                                    ...
                                    Iterator iterator =
                                            this.tileEntityData.func_150296_c().iterator();
                            ...
                            this.entityDropItem(
                                    new ItemStack(this.block, 1, this.block.damageDropped(this.blockMeta)), 0.0F);
                        }
                    }
        ...
        if (canDestroy(b)
                && (b.isAir(world, x, y, z)
                        || !b.isOpaqueCube()
                                ...
                                && !worldObj.isSideSolid(x, y, z, ForgeDirection.UP)))
    ...
    protected void fall(float fallDistance) {
        if (this.hurtEntities) {
            ...
            if (height > 0) {
                ArrayList arraylist =
                        new ArrayList(this.worldObj.getEntitiesWithinAABBExcludingEntity(this, this.boundingBox));
                DamageSource damagesource =
                        new DamageSource("caveIn").setDamageBypassesArmor().setDamageIsAbsolute();
                Iterator iterator = arraylist.iterator();

                while (iterator.hasNext()) {
                    Entity entity = (Entity) iterator.next();
                    entity.attackEntityFrom(
                            damagesource, Math.min(MathHelper.floor_float(height * this.damage), this.maxDamage));
                }
            }
        }
    }
