...
public class EntityFishHookTFC extends EntityFishHook {
    ...
    public void onUpdate() {
        ...
        if (this.fishPosRotationIncrements > 0) {
            ...
            this.rotationPitch = (float)
                    (this.rotationPitch + (this.fishPitch - this.rotationPitch) / this.fishPosRotationIncrements);
            ...
            if (!this.worldObj.isRemote) {
                ...
                if (this.field_146042_b.isDead
                        || !this.field_146042_b.isEntityAlive()
                        || itemstack == null
            ...
            if (!this.inGround) {
                ...
                for (int k = 0; k < b0; ++k) {
                    double d7 = this.boundingBox.minY
                            + (this.boundingBox.maxY - this.boundingBox.minY) * (k + 0) / b0
                            - 0.125D
                            + 0.125D;
                    double d8 = this.boundingBox.minY
                            + (this.boundingBox.maxY - this.boundingBox.minY) * (k + 1) / b0
                            - 0.125D
                            + 0.125D;
    ...
    public Vec3 getNormalDirectionOfPlayer(double x, double y, double z) {
        Vec3 dirVec = Vec3.createVectorHelper(
                this.field_146042_b.posX - x, this.field_146042_b.posY - y, this.field_146042_b.posZ - z);
        return dirVec.normalize();
    }

    public Vec3 getTooFarAdjustedVec(Vec3 motionVec, double x, double y, double z) {
        Vec3 playerMotion = Vec3.createVectorHelper(
                this.field_146042_b.motionX, this.field_146042_b.motionY, this.field_146042_b.motionZ);
        double subractedRatio = Math.max(
                1
                        - (this.maxDistance
                                / this.field_146042_b.getDistance(
                                        x + playerMotion.xCoord, y + playerMotion.yCoord, z + playerMotion.zCoord)),
                0);
        return Vec3.createVectorHelper(
