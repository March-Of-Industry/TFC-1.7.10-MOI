...
public class EntityAIPanicTFC extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        if (this.theEntityCreature.getAITarget() == null
                ...
                && (this.theEntityCreature instanceof IAnimal
                                && ((IAnimal) this.theEntityCreature).getAttackedVec() == null
                        || !(this.theEntityCreature instanceof IAnimal))) {
            return false;
        } else {
            ...
            if (vec3 == null) {
                ...
                if (alertHerd && this.theEntityCreature instanceof IAnimal) {
                    List list = this.theEntityCreature.worldObj.getEntitiesWithinAABB(
                            this.theEntityCreature.getClass(), this.theEntityCreature.boundingBox.expand(8, 8, 8));
    ...
    public Vec3 updateAttackVec(IAnimal theCreature, Vec3 attackedVec) {
        if (theCreature.getFearSource() != null
                && TFC_Core.getEntityPos(theEntityCreature).distanceTo(attackedVec)
                        > this.theEntityCreature.getDistanceToEntity(theCreature.getFearSource())) {
