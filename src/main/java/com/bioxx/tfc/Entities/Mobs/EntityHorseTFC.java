...
public class EntityHorseTFC extends EntityHorse implements IInvBasic, IAnimal {
    ...
    public EntityHorseTFC(World par1World) {
        ...
        sizeMod = (float) Math.sqrt(
                ((rand.nextInt(rand.nextInt((DEGREE_OF_DIVERSION + 1) * 10) + 1) * (rand.nextBoolean() ? 1 : -1))
                                        * 0.01f
                                + 1F)
                        * (1.0F - DIMORPHISM * sex));
    ...
    public EntityHorseTFC(World par1World, IAnimal mother, List<Float> data, int type, int variant) {
        ...
        sizeMod = (float)
                Math.sqrt(sizeMod * sizeMod * (float) Math.sqrt((mother.getSizeMod() + fatherSize) * invSizeRatio));
        strengthMod = (float)
                Math.sqrt(strengthMod * strengthMod * (float) Math.sqrt((mother.getStrengthMod() + fatherStr) * 0.5F));
        aggressionMod = (float) Math.sqrt(
                aggressionMod * aggressionMod * (float) Math.sqrt((mother.getAggressionMod() + fatherAggro) * 0.5F));
        obedienceMod = (float) Math.sqrt(
                obedienceMod * obedienceMod * (float) Math.sqrt((mother.getObedienceMod() + fatherObed) * 0.5F));

        this.familiarity =
                (int) (mother.getFamiliarity() < 90 ? mother.getFamiliarity() / 2 : mother.getFamiliarity() * 0.9f);
    ...
    public EntityAgeable createChildTFC(EntityAgeable eAgeable) {
        ...
        double jumpSum = this.getEntityAttribute(HORSE_JUMP_STRENGTH).getBaseValue()
                + this.mateJumpStrength
        ...
        baby.setHealth((float)
                baby.getEntityAttribute(SharedMonsterAttributes.maxHealth).getBaseValue());
        return baby;
    }
    ...
    protected void entityInit() {
        ...
        this.dataWatcher.addObject(
                25, String.valueOf("0")); // Time of conception, stored as a string since we can't do long
    }

    @Override
    public void familiarize(EntityPlayer ep) {
        ItemStack stack = ep.getHeldItem();
        if ((this.riddenByEntity == null || !this.riddenByEntity.equals(ep))
                && !familiarizedToday
                && stack != null
                && this.isFood(stack)
                && canFamiliarize()) {
    ...
    protected EntityHorseTFC getClosestHorse(Entity entity, double range) {
        ...
        List<EntityHorseTFC> list = this.worldObj.getEntitiesWithinAABBExcludingEntity(
                entity, entity.boundingBox.addCoord(range, range, range), HORSE_BREEDING_SELECTOR);
    ...
    public boolean getLeashed() {
        if (super.getLeashed()
                && getLeashedToEntity() instanceof EntityPlayer
    ...
    public int increaseTemper(int temper) {
        ...
        int j = MathHelper.clamp_int(
                this.getTemper() + (int) (temper * obedienceMod * (1f / aggressionMod)), 0, this.getMaxTemper());
        this.setTemper(j);
        return j;
    }
    ...
    public boolean interact(EntityPlayer player) {
        ...
        if (itemstack != null
                && this.isBreedingItemTFC(itemstack)
            ...
            return true;
                ...
                && itemstack.getItem() instanceof ItemCustomNameTag
            ...
            if (itemstack != null) {
                ...
                if (this.func_110229_cs() /*Donkey or Mule*/
                        && !this.isChested()
                        && itemstack.getItem() == Item.getItemFromBlock(TFCBlocks.chest)) {
                    this.setChested(true);
                    this.playSound(
                            "mob.chickenplop", 1.0F, (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F);
                    flag = true;
                    this.updateChestSaddle();
                }
    ...
    public boolean isFood(ItemStack item) {
        return item != null
                && (item.getItem() == TFCItems.wheatGrain
                        || item.getItem() == TFCItems.oatGrain
    ...
    public void mate(IAnimal otherAnimal) {
        ...
        mateMaxHealth = ((EntityLivingBase) otherAnimal)
                .getEntityAttribute(SharedMonsterAttributes.maxHealth)
                .getBaseValue();
        mateJumpStrength = ((EntityLivingBase) otherAnimal)
                .getEntityAttribute(HORSE_JUMP_STRENGTH)
                .getBaseValue();
        mateMoveSpeed = ((EntityLivingBase) otherAnimal)
    ...
    public void openGUI(EntityPlayer player) {
        Entity temp =
                player.ridingEntity; // Trick the player into thinking they are riding the horse so we can track that
        // specific horse
        ...
        if (temp == null) // Shift-right click while not riding to open inventory
        {
            ...
            if (player instanceof EntityPlayerMP) {
                ...
                playerMP.playerNetServerHandler.sendPacket(
                        new S1BPacketEntityAttach(0, playerMP, this)); // Sets ridingEntity to this for playerMP
                openHorseContainer(player);
                playerMP.playerNetServerHandler.sendPacket(
                        new S1BPacketEntityAttach(0, playerMP, temp)); // Resets ridingEntity for playerMP
            } else {
                openHorseContainer(player);
            }
    ...
    private void openHorseContainer(EntityPlayer player) {
        if (!this.worldObj.isRemote
                ...
                && this.isTame()) {
            player.openGui(TerraFirmaCraft.instance, 42, player.worldObj, (int) player.posX, (int) player.posY, (int)
                    player.posZ);
        }
    }
