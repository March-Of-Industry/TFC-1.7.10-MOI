...
public class EntityFishTFC extends EntitySquid {
    ...
    public boolean getCanSpawnHere() {
        return this.posY > Global.SEALEVEL - 16
                && this.posY <= Global.SEALEVEL
    ...
    public boolean isInWater() {
        return this.worldObj.handleMaterialAcceleration(
                this.boundingBox.expand(0.0D, -0.0200000238418579D, 0.0D), Material.water, this);
    }
    ...
    public void onLivingUpdate() {
        ...
        if (!worldObj.isRemote) {
            ...
            if (hooked
                    && this.riddenByEntity instanceof EntityFishHookTFC
                ...
                if (pullX != 0
                        && TFC_Core.isWater(worldObj.getBlock(
                                (int) (this.posX - (pullX / Math.abs(pullX))), (int) this.posY, (int) this.posZ))) {
                ...
                if (pullZ != 0
                        && TFC_Core.isWater(worldObj.getBlock(
                                (int) this.posX, (int) this.posY, (int) (this.posZ - (pullZ / Math.abs(pullZ)))))) {
                ...
                if (pullX != 0
                        && TFC_Core.isWater(worldObj.getBlock(
                                (int) this.posX, (int) (this.posY - (pullY / (Math.abs(pullY)))), (int) this.posZ))) {
                    randY = 1 - (randX + randZ);
                } else {
                    randY = 0;
                }
            ...
            if (this.isInWater()) {
                ...
                this.squidPitch +=
                        (-((float) Math.atan2(f, this.motionY)) * 180.0F / (float) Math.PI - this.squidPitch) * 0.1F;
            } else {
            ...
            if (hooked && this.riddenByEntity instanceof EntityFishHookTFC) {
                ...
                for (int i = 0; i < 1; i++) {
                    if (fh.isTooFarFromPlayer(posX, posY, posZ)) {
                        Vec3 dirVec = fh.getTooFarAdjustedVec(
                                Vec3.createVectorHelper(motionX, motionY, motionZ), posX, posY, posZ);
            ...
            this.currentRenderPitch = xzMotion != 0
                    ? (float) (Math.atan2(renderMotionY, xzMotion) * 180.0D / Math.PI)
                    : currentRenderPitch;
    ...
    private int[] getRandomDestination(double x, double y, double z) {
        ...
        boolean needsNewLocation =
                this.rand.nextInt(25) < 3 /*) || !(TFC_Core.isWater(this.worldObj.getBlock(destX, destY-1, destZ))*/
                        || tooCloseToPlayer;
            ...
            destX = (int) x
                    + this.rand.nextInt(10)
                            * (this.rand.nextBoolean()
                                    ? (validDirs[1] ? -1 : validDirs[0] ? 1 : 0)
                                    : validDirs[0] ? 1 : validDirs[1] ? -1 : 0);
            destY = (int) y
                    + this.rand.nextInt(3)
                            * (this.rand.nextBoolean()
                                    ? (validDirs[3] ? -1 : validDirs[2] ? 1 : 0)
                                    : validDirs[2] ? 1 : validDirs[3] ? -1 : 0);
            destZ = (int) z
                    + this.rand.nextInt(10)
                            * (this.rand.nextBoolean()
                                    ? (validDirs[5] ? -1 : validDirs[4] ? 1 : 0)
