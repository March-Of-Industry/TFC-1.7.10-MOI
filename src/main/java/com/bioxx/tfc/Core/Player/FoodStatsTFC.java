...
public class FoodStatsTFC {
    private boolean updateStats =
            !TFCOptions.enableDebugMode; // Replace with true to allow stat depleting with debug mode enabled
    ...
    private ResourceLocation wastedBlur =
            new ResourceLocation("shaders/post/blur.json"); // new ResourceLocation("shaders/post/blurPhosphor.json");
    ...
    public void onUpdate(EntityPlayer player) {
        if (!player.worldObj.isRemote) {
            ...
            if (TFC_Time.getTotalTicks() - this.foodTimer >= TFC_Time.HOUR_LENGTH
                ...
                if (this.stomachLevel <= 0) {
                    ...
                    if (this.satProtein)
                        this.addNutrition(
                                EnumFoodGroup.Protein, this.satisfaction * ((1 - this.nutrProtein) / 100), false);
                    if (this.satGrain)
                        this.addNutrition(EnumFoodGroup.Grain, this.satisfaction * ((1 - this.nutrGrain) / 100), false);
                    if (this.satVeg)
                        this.addNutrition(
                                EnumFoodGroup.Vegetable, this.satisfaction * ((1 - this.nutrVeg) / 100), false);
                    if (this.satFruit)
    ...
    public void clientUpdate() {
        if (Minecraft.getMinecraft().entityRenderer instanceof EntityRendererTFC) {
            ...
            if ((erTFC.getCurrentShaderLocation() == null
                            || !erTFC.getCurrentShaderLocation().equals(wastedBlur))
                    && soberTime > TFC_Time.getTotalTicks() + 8000) {
                erTFC.setManualShader(wastedBlur);
            } else if ((erTFC.getCurrentShaderLocation() == null
                            || !erTFC.getCurrentShaderLocation().equals(drunkBlur))
                    && soberTime > TFC_Time.getTotalTicks() + 4000
                    && soberTime <= TFC_Time.getTotalTicks() + 8000) {
