...
public class TFC_ConfigGUI extends GuiConfig {
    ...
    private static List<IConfigElement> getConfigElements() {
        ...
        root.add(new DummyCategoryElement(
                "TFCOptions.cfg", "config.gui.TFCConfig", getAllFrom(TFC_ConfigFiles.getGeneralConfig())));
        root.add(new DummyCategoryElement(
                "TFCCrafting.cfg", "config.gui.TFCCrafting", getAllFrom(TFC_ConfigFiles.getCraftingConfig())));
        root.add(new DummyCategoryElement(
                "TFCOre.cfg", "config.gui.TFCOre", getAllFrom(TFC_ConfigFiles.getOresConfig())));
        return root;
    }
    ...
    private static List<IConfigElement> getAllFrom(Configuration generalConfig) {
        ...
        for (String catName : generalConfig.getCategoryNames()) {
            if (catName.contains(Configuration.CATEGORY_SPLITTER))
                continue; // getCategoryNames includes sub categories, which we don't want.
