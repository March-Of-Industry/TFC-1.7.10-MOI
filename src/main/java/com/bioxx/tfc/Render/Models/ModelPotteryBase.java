...
public class ModelPotteryBase extends ModelBox {
    ...
    public PositionTextureVertex[] newRing(
            float originX, float originY, float originZ, float offsetX, float offsetY, float offsetZ, float width) {
        ...
        vert[0] = new PositionTextureVertex(
                originX + offsetX - (width / 2) - widthX, originY + offsetY, originZ + offsetZ - (width / 2f), 0, 0);
        vert[1] = new PositionTextureVertex(
                originX + offsetX + (width / 2) - widthX, originY + offsetY, originZ + offsetZ - (width / 2f), 0, 0);
        vert[2] = new PositionTextureVertex(
                originX + offsetX + (width / 2f), originY + offsetY, originZ + offsetZ - (width / 2) - widthX, 0, 0);
    ...
    public List<TexturedQuad> buildSides(
            Object[] vertices,
            ModelRenderer renderer,
            int textureOffsetX,
            int textureOffsetY,
        ...
        for (int i = 0; i < vertices.length - 1; i++) {
            for (int j = 0; j < 8; j++) {
                aList.add(new TexturedQuad(
                        new PositionTextureVertex[] {
                            ((PositionTextureVertex[]) (vertices[i + 1]))[j],
