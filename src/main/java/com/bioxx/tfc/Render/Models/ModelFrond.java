...
public class ModelFrond extends ModelBox {
        ...
        */
        ...
        for (int i = 0; i < 6; i++) {
            blades[i + 24] = createBlade(
                    originX, originY, originZ, (2 - (float) Math.pow(i, 2) / 4) * lengthMod, 0, 1.2f * (i + 24));
        }
    ...
    private PositionTextureVertex[] createBlade(
            float originX, float originY, float originZ, float modifierX, float modifierY, float modifierZ) {
        ...
        PositionTextureVertex vert1 = new PositionTextureVertex(
                originX + length + modifierX, originY + modifierY, originZ + modifierZ + (modifierZ / 15f), 0.0F, 8.0F);
