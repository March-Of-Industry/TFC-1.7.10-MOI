...
public class TESRHopper extends TESRBase {
    ...
    public void renderPress(Block block, World world, int x, int y, int z, int meta) {
        ...
        renderBlocks.renderFaceYNeg(
                block, -0.5D, -0.5D, -0.5D, renderBlocks.getBlockIconFromSideAndMetadata(block, 0, meta));
        tessellator.setColorOpaque_F(f1, f1, f1);
        renderBlocks.renderFaceYPos(
                block, -0.5D, -0.5D, -0.5D, renderBlocks.getBlockIconFromSideAndMetadata(block, 1, meta));
        tessellator.setColorOpaque_F(f2, f2, f2);
        renderBlocks.renderFaceZNeg(
                block, -0.5D, -0.5D, -0.5D, renderBlocks.getBlockIconFromSideAndMetadata(block, 2, meta));
        tessellator.setColorOpaque_F(f2, f2, f2);
        renderBlocks.renderFaceZPos(
                block, -0.5D, -0.5D, -0.5D, renderBlocks.getBlockIconFromSideAndMetadata(block, 3, meta));
        tessellator.setColorOpaque_F(f3, f3, f3);
        renderBlocks.renderFaceXNeg(
                block, -0.5D, -0.5D, -0.5D, renderBlocks.getBlockIconFromSideAndMetadata(block, 4, meta));
        tessellator.setColorOpaque_F(f3, f3, f3);
        renderBlocks.renderFaceXPos(
                block, -0.5D, -0.5D, -0.5D, renderBlocks.getBlockIconFromSideAndMetadata(block, 5, meta));
        tessellator.draw();
    }
