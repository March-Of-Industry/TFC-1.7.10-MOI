...
public class RenderSupportBeam implements ISimpleBlockRenderingHandler {
    public static boolean renderWoodSupportBeamH(Block block, int i, int j, int k, RenderBlocks renderblocks) {
        ...
        // X
        if (
        /*TFC_Core.isBlockAboveSolid(blockAccess, i, j, k) &&*/ TFCBlocks.isBlockVSupport(
                        blockAccess.getBlock(i - 1, j, k))
                || TFCBlocks.isBlockHSupport(blockAccess.getBlock(
                        i - 1, j, k))) // if the block above is solid and the block at -x is a support beam
        {
        ...
        /*TFC_Core.isBlockAboveSolid(blockAccess, i, j, k) &&*/ TFCBlocks.isBlockVSupport(
                        blockAccess.getBlock(i + 1, j, k))
                || TFCBlocks.isBlockHSupport(blockAccess.getBlock(i + 1, j, k))) {
            if (hasVerticalBeam) // if the block does contain a vertical beam
            {
                renderblocks.setRenderBounds(0.75F, 0.50F, 0.25F, 1.0F, 1.0F, 0.75F);
                renderblocks.renderStandardBlock(block, i, j, k);
        ...
        if (
        /*TFC_Core.isBlockAboveSolid(blockAccess, i, j, k) &&*/ TFCBlocks.isBlockVSupport(
                        blockAccess.getBlock(i, j, k - 1))
                || TFCBlocks.isBlockHSupport(blockAccess.getBlock(i, j, k - 1))) {
            if (TFCBlocks.isBlockVSupport(blockAccess.getBlock(i, j, k + 1))
                    || TFCBlocks.isBlockHSupport(blockAccess.getBlock(
                            i, j, k + 1))) // if the block above is solid and the block at +x is a support beam
            {
                if (hasVerticalBeam) // if the block does contain a vertical beam
                {
        ...
        /*TFC_Core.isBlockAboveSolid(blockAccess, i, j, k) && */ TFCBlocks.isBlockVSupport(
                        blockAccess.getBlock(i, j, k + 1))
                || TFCBlocks.isBlockHSupport(
                        blockAccess.getBlock(i, j, k + 1))) // Top is solid and positive Z is support
        {
            if (hasVerticalBeam) // if the block does contain a vertical beam
            {
                renderblocks.setRenderBounds(0.25F, 0.50F, 0.75F, 0.75F, 1.0F, 1.0F);
    ...
    public static boolean renderWoodSupportBeamV(Block block, int i, int j, int k, RenderBlocks renderblocks) {
        ...
        if ((blockAccess.getBlock(i, j - 1, k).isOpaqueCube()
                        || TFCBlocks.isBlockVSupport(blockAccess.getBlock(i, j - 1, k)))
    ...
    public static boolean isSurroundedByWater(IBlockAccess access, int i, int j, int k) {
        if (access.getBlock(i, j + 1, k).getMaterial() == Material.water) return true;

        return access.getBlock(i + 1, j, k).getMaterial() == Material.water
