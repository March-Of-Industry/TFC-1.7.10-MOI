...
public class RenderOverlayHandler {
    public static ResourceLocation tfcicons =
            new ResourceLocation(Reference.MOD_ID, Reference.ASSET_PATH_GUI + "icons.png");
    private FontRenderer fontrenderer;
    public int recordTimer;
    private final Field _recordPlayingUpFor =
            ReflectionHelper.findField(GuiIngame.class, "recordPlayingUpFor", "field_73845_h");
    ...
    public void render(RenderGameOverlayEvent.Pre event) {
            ...
            if (currentItem instanceof ItemCustomHoe) {
                ...
                if (hasHammer) {
                    ...
                    this.drawTexturedModalRect(
                            ...
                            ChiselManager.getInstance().getTextureV(mode),
                            20,
                            20);
                    TFC_Core.bindTexture(tfcicons);
                }
        ...
        if (quiver instanceof ItemQuiver) {
            ...
            if (leftSide) {
                ...
                fontrenderer.drawString(
                        Integer.toString(getQuiverJavelins()), xPos + 19, yPos + 21, Color.white.getRGB());
            } else {
                ...
                fontrenderer.drawString(
                        Integer.toString(getQuiverArrows()), xPos - arrowOffset, yPos + 4, Color.white.getRGB());
                fontrenderer.drawString(
                        Integer.toString(getQuiverJavelins()), xPos - javOffset, yPos + 21, Color.white.getRGB());
            }
        ...
        if (mc.playerController.gameIsSurvivalOrAdventure()) {
            ...
            if (playerInfo != null && playerInfo.guishowFoodRestoreAmount) {
                float percentFood2 =
                        Math.min(percentFood + playerInfo.guiFoodRestoreAmount / foodstats.getMaxStomach(player), 1);
            ...
            if (player.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
                            .getModifier(TFCAttributes.OVERBURDENED_UUID)
                    != null)
                mc.fontRenderer.drawString(
    ...
    protected void renderDismountOverlay(Minecraft mc, int midpoint, int height, float partialTicks) {
        if (recordTimer == 0) {
            try {
                ...
                _recordPlayingUpFor.setInt(
                        mc.ingameGUI, 0); // Reset the vanilla timer so the vanilla overlay doesn't appear
