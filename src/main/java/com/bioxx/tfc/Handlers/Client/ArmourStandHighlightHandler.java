...
public class ArmourStandHighlightHandler {
    ...
    public void drawBlockHighlightEvent(DrawBlockHighlightEvent evt) {
        ...
        if (evt.currentItem == null
            ...
            vectorTerm =
                    vectorTerm.addVector(evt.target.hitVec.xCoord, evt.target.hitVec.yCoord, evt.target.hitVec.zCoord);
            ...
            if (boxToRender != null) {
                ...
                drawOutlinedBoundingBox(
                        boxToRender.expand(0.02F, 0.02F, 0.02F).getOffsetBoundingBox(-var8, -var10, -var12));
                GL11.glDepthMask(true);
    ...
    public boolean isVecInsideBox(
            AxisAlignedBB aabb, EntityPlayer player, Vec3 unit, double xOffset, double yOffset, double zOffset) {
        unit = player.getLookVec();
        aabb.minY += 0.1;
        aabb.maxY += 0.1;
        ...
        Vec3 distBlockxyz = playerVec
                .subtract(Vec3.createVectorHelper(aabb.minX, aabb.minY, aabb.minZ))
                .normalize();
        Vec3 distBlockXYZ = playerVec
                .subtract(Vec3.createVectorHelper(aabb.maxX, aabb.maxY, aabb.maxZ))
                .normalize();

        Vec3 distBlockxyZ = playerVec
                .subtract(Vec3.createVectorHelper(aabb.minX, aabb.minY, aabb.maxZ))
                .normalize();
        Vec3 distBlockXYz = playerVec
                .subtract(Vec3.createVectorHelper(aabb.maxX, aabb.maxY, aabb.minZ))
                .normalize();

        Vec3 distBlockxYZ = playerVec
                .subtract(Vec3.createVectorHelper(aabb.minX, aabb.maxY, aabb.maxZ))
                .normalize();
        Vec3 distBlockXyz = playerVec
                .subtract(Vec3.createVectorHelper(aabb.maxX, aabb.minY, aabb.minZ))
                .normalize();

        Vec3 distBlockxYz = playerVec
                .subtract(Vec3.createVectorHelper(aabb.minX, aabb.maxY, aabb.minZ))
                .normalize();
        Vec3 distBlockXyZ = playerVec
                .subtract(Vec3.createVectorHelper(aabb.maxX, aabb.minY, aabb.maxZ))
                .normalize();
