...
public class ChunkEventHandler {
    ...
    public void onLoad(ChunkEvent.Load event) {
        if (!event.world.isRemote && TFC_Core.getCDM(event.world) != null && event.getChunk() != null) {
            ...
            if (cd == null) return;
            BiomeGenBase biome =
                    event.world.getBiomeGenForCoords(event.getChunk().xPosition, event.getChunk().zPosition);
            ...
            data.rainfallMap = TFC_Climate.getCacheManager(event.world)
                    .loadRainfallLayerGeneratorData(
                            data.rainfallMap, event.getChunk().xPosition * 16, event.getChunk().zPosition * 16, 16, 16);
    ...
    public void onUnload(ChunkEvent.Unload event) {
        if (TFC_Core.getCDM(event.world) != null
            ...
            TFC_Core.getCDM(event.world).getData(event.getChunk().xPosition, event.getChunk().zPosition).isUnloaded =
                    true;
    }
