        ...
        if (soberTime > TFC_Time.getTotalTicks()) {
            ...
            for (int i = 0; i < event.message.length() - 1; i++) {
                ...
                if (event.message.charAt(0) != '/') {
                    int chance =
                            Math.max(1, 11 - (int) (soberTime / 1000)); // Can't run rand.nextInt on a negative number.
                    if (s.indexOf(s2) != -1 && rand.nextInt(chance) == 0) {
                        ...
                        for (int j = 0; j < n; j++) {
                            msg = msg
                                    + (end.substring(0, 1).toUpperCase().equals(end.substring(0, 1))
                                            ? s2
                                            : s2.toLowerCase());
                        }
