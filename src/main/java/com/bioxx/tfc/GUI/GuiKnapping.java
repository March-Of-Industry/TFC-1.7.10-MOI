...
public class GuiKnapping extends GuiContainerTFC {
    ...
    public static ResourceLocation texture =
            new ResourceLocation(Reference.MOD_ID, Reference.ASSET_PATH_GUI + "gui_knapping.png");
    private final Field _selectedButton =
            ReflectionHelper.findField(GuiScreen.class, "selectedButton", "field_146290_a");
    ...
    public void initGui() {
        ...
        for (int y = 0; y < 5; y++) {
            for (int x = 0; x < 5; x++) {
                buttonList.add(
                        new GuiKnappingButton(x + (y * 5), guiLeft + (x * 16) + 10, guiTop + (y * 16) + 12, 16, 16));
                // Normal Behavior
                if (!previouslyLoaded) {
                    ...
                    if (guiButton.mousePressed(this.mc, mouseX, mouseY)) {
                        try {
                            ActionPerformedEvent.Pre event =
                                    new ActionPerformedEvent.Pre(this, guiButton, this.buttonList);
                            ...
                            if (this.equals(this.mc.currentScreen))
                                MinecraftForge.EVENT_BUS.post(
                                        new ActionPerformedEvent.Post(this, event.button, this.buttonList));
                        } catch (Exception e) {
