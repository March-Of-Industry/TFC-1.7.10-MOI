...
public class GuiPlanButton extends GuiButton {
    ...
    public GuiPlanButton(
            int index, int xPos, int yPos, int width, int height, ItemStack ico, GuiPlanSelection gui, String s) {
    ...
    public void drawButton(Minecraft mc, int x, int y) {
        if (this.visible) {
            ...
            this.field_146123_n = isPointInRegion(
                    x, y); // x >= this.xPosition && y >= this.yPosition && x < this.xPosition + this.width && y <
    ...
    private boolean isPointInRegion(int mouseX, int mouseY) {
        ...
        mouseX -= k1;
        mouseY -= l1;
        return mouseX >= xPosition - 1
                && mouseX < xPosition + width + 1
                && mouseY >= yPosition - 1
