...
public class GuiContainerCreativeTFC extends InventoryEffectRenderer {
    ...
    protected void handleMouseClick(Slot par1Slot, int par2, int par3, int par4) {
        ...
        if (par1Slot == null && selectedTabIndex != CreativeTabs.tabInventory.getTabIndex() && par4 != 5) {
            ...
            if (par1Slot == this.zeroSlot && flag) {
                for (l = 0;
                        l < this.mc.thePlayer.inventoryContainer.getInventory().size();
                        ++l) {
                ...
                if (selectedTabIndex == CreativeTabs.tabInventory.getTabIndex()) {
                    if (par1Slot == this.zeroSlot) {
                        ...
                        itemstack1 = par1Slot.decrStackSize(
                                par3 == 0 ? 1 : par1Slot.getStack().getMaxStackSize());
                        ...
                        this.mc.playerController.sendPacketDropItem /*func_78752_a*/(
                        ...
                        this.mc.thePlayer.inventoryContainer.slotClick(
                                par1Slot == null
                                        ? par2
                                        : SlotCreativeInventoryTFC.getSlot((SlotCreativeInventoryTFC) par1Slot)
                                                .slotNumber,
                                par3,
                                par4,
                                this.mc.thePlayer);
                    ...
                    if (itemstack != null
                            ...
                            && ItemStack.areItemStackTagsEqual(
                                    itemstack,
                                    itemstack2)) // Forge: Bugfix, Compare NBT data, allow for deletion of enchanted
                    // books, MC-12770
                    {
                        if (par3 == 0) {
                    ...
                    this.inventorySlots.slotClick(
                            par1Slot == null ? par2 : par1Slot.slotNumber, par3, par4, this.mc.thePlayer);

                    if (Container.func_94532_c(par3) == 2) {
                        for (l = 0; l < 9; ++l)
                            this.mc.playerController.sendSlotPacket(
                                    this.inventorySlots.getSlot(45 + l).getStack(), 36 + l);
                    } else if (par1Slot != null) {
                        itemstack1 =
                                this.inventorySlots.getSlot(par1Slot.slotNumber).getStack();
                        this.mc.playerController.sendSlotPacket(
                                itemstack1, par1Slot.slotNumber - this.inventorySlots.inventorySlots.size() + 9 + 36);
                    }
                }
            }
        }
    ...
    public void initGui() {
        if (this.mc.playerController.isInCreativeMode()) {
            ...
            this.searchField = new GuiTextField(
                    this.fontRendererObj, this.guiLeft + 82, this.guiTop + 6, 89, this.fontRendererObj.FONT_HEIGHT);
        ...
        while (iterator.hasNext()) {
            ...
            while (loop) {
                if (iterator1.hasNext()) {
                    ...
                    if (!s1.toLowerCase().contains(s)) continue;
                    flag = true;
                }
    ...
    private void setCurrentCreativeTab(CreativeTabs par1CreativeTabs) {
        ...
        if (par1CreativeTabs == CreativeTabs.tabInventory) {
            ...
            for (int j = 0; j < container.inventorySlots.size(); ++j) {
                SlotCreativeInventoryTFC slotcreativeinventory =
                        new SlotCreativeInventoryTFC(this, (Slot) container.inventorySlots.get(j), j);
    ...
    public void drawScreen(int par1, int par2, float par3) {
        ...
        if (this.zeroSlot != null
                && selectedTabIndex == CreativeTabs.tabInventory.getTabIndex()
                && this.func_146978_c /*isPointInRegion*/(
                        this.zeroSlot.xDisplayPosition, this.zeroSlot.yDisplayPosition, 16, 16, par1, par2)) {
    ...
    protected void renderToolTip(ItemStack par1ItemStack, int par2, int par3) {
        if (selectedTabIndex == CreativeTabs.tabAllSearch.getTabIndex()) {
            ...
            if (creativetabs == null && par1ItemStack.getItem() == Items.enchanted_book) {
                ...
                if (map.size() == 1) {
                    Enchantment enchantment = Enchantment.enchantmentsList[
                            ((Integer) map.keySet().iterator().next()).intValue()];
                    CreativeTabs[] acreativetabs = CreativeTabs.creativeTabArray;
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
        ...
        if (creativetabs == CreativeTabs.tabInventory)
            GuiInventoryTFC.drawPlayerModel(
                    this.guiLeft + 43,
                    this.guiTop + 45,
                    20,
