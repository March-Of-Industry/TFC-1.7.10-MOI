    ...
    public void processCommand(ICommandSender sender, String[] params) {
        ...
        if (params.length >= 2) {
            ...
            if (params[0].equals("biome")) {
                ...
                drawBiomeImage(
                        (int) Math.floor(player.posX), (int) Math.floor(player.posZ), size, world, name, skipSize);
        ...
        try {
            ...
            for (int x = -sizeHalf; x < sizeHalf; x++) {
                for (int z = -sizeHalf; z < sizeHalf; z++) {
                    count++;
                    int temp = (int) (255
                            * (TFC_Climate.getBioTemperature(world, xCoord + x * skipSize, zCoord + z * skipSize)
                                    / 50));
                ...
                for (int z = -size / 2; z < size / 2; z++) {
                    ...
                    graphics.setColor(Color.getColor(
                            ...
                            ((TFCBiome) world.getWorldChunkManager()
                                            .getBiomeGenAt(x * skipSize + xCoord, z * skipSize + zCoord))
                                    .getBiomeColor()));
